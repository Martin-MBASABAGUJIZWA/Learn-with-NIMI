...
function ConfirmDialog({ title, message, confirmLabel, cancelLabel = 'Cancel', danger = true, alertOnly = false, onConfirm, onCancel }: ConfirmDialogProps) {
  ...
  const tint = alertOnly || !danger ? 'green' : 'red'
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-ds-card border border-ds-border max-w-sm w-full p-6"
        onClick={(e) => e.stopPropagation()}
        role="alertdialog"
        aria-modal="true"
        aria-labelledby="confirm-dialog-title"
      >
        <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 ${tint === 'red' ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-600'}`}>
          {alertOnly ? <Info className="w-6 h-6" /> : <AlertTriangle className="w-6 h-6" />}
        </div>
        <h3 id="confirm-dialog-title" className="text-base font-bold text-gray-900 mb-1.5">{title}</h3>
