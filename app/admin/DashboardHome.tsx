...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  // — Realtime subscriptions for live stat updates —
  useEffect(() => {
    async function fetchParentsCount() {
      const { count } = await supabase.from('parents').select('id', { count: 'exact', head: true })
      // parents count not shown directly in stats, but drives families tab — skip updating stats here
      // (If you add a parents stat card, update it here)
    }

    async function fetchChildrenCount() {
      const { count } = await supabase.from('children').select('id', { count: 'exact', head: true })
      setStats(prev => ({ ...prev, children: count ?? prev.children }))
    }

    async function fetchClubCount() {
      const { count } = await supabase.from('nimipiko_subscriptions').select('id', { count: 'exact', head: true }).eq('status', 'active')
      setRevenue(prev => ({ ...prev, activeSubscriptions: count ?? prev.activeSubscriptions }))
    }

    async function fetchCertsCount() {
      const { count } = await supabase.from('child_achievements').select('id', { count: 'exact', head: true }).eq('type', 'certificate')
      setStats(prev => ({ ...prev, certs: count ?? prev.certs }))
    }

    const channel = supabase
      .channel('dashboard-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'parents' }, () => {
        void fetchParentsCount()
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'children' }, () => {
        void fetchChildrenCount()
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'nimipiko_subscriptions' }, () => {
        void fetchClubCount()
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'child_achievements' }, () => {
        void fetchCertsCount()
      })
      .subscribe()
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2.5">
          <span className="text-[12px] font-bold text-gray-500 uppercase tracking-wide">Overview</span>
