...
function SlugInput({ storyId, initialSlug, titleHint, onSaved }: { storyId: string; initialSlug: string; titleHint: string; onSaved: () => void }) {
  ...
  // Auto-fill slug from title when slug is still auto-generated and title is meaningful.
  // On conflict the DB returns an error; try appending '-story' as a fallback.
  useEffect(() => {
    ...
    // Fire the save; if it conflicts, SlugInput will show the error inline
    // and the admin can then edit manually or we retry with '-story' suffix
    ...
    timer.current = setTimeout(async () => {
      ...
      if (dbErr) {
        // Conflict: try with '-story' suffix
