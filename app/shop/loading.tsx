    ...
    <AppShell>
      <div className="max-w-5xl mx-auto w-full pb-24 space-y-6">
        ...
        {Array.from({ length: 3 }).map((_, s) => (
          <div key={s} className="leaf-lg border border-[var(--ds-border-primary)] p-5 space-y-4">
            <Bone className="h-6 w-40" />
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="leaf border border-[var(--ds-border-primary)] p-3 space-y-3">
