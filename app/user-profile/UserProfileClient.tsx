...
// ── Main page ─────────────────────────────────────────────────────────
export default function UserProfileClient({ initialChildren, initialHasSubscription }: Props = {}) {
  const { t } = useLanguage();
  ...
  const prefetchedChildrenRef = useRef<Child[] | undefined>(initialChildren);

  const loadProgress = useCallback(async (targetChildId?: string, silent = false) => {
    const gen = silent ? ++switchGenRef.current : 0;
    if (silent) setRefreshing(true); else setLoading(true);
    // Use server-pre-fetched children on the first call (no targetChildId, not a silent refresh)
    let list: Child[];
    const prefetched = prefetchedChildrenRef.current;
    if (prefetched !== undefined && !targetChildId && !silent) {
      list = prefetched;
      prefetchedChildrenRef.current = undefined;
    } else {
    ...
    // Skip if we have a pre-fetched subscription value from the server.
    const prefetchedSub = initialHasSubscription;
