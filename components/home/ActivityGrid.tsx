        ...
        return (
          <Link key={activity.number} href={activity.href}>
            <motion.div
              ...
              className="group relative overflow-hidden border border-[var(--ds-border-primary)] bg-[var(--ds-surface-card)]/90 shadow-[0_14px_32px_rgba(15,23,42,0.08)] hover:shadow-[0_18px_38px_rgba(15,23,42,0.12)] transition-all cursor-pointer flex flex-col items-center pt-3 pb-3 px-2 h-full"
              style={{
            ...
            >
              ...
              <div className="absolute inset-x-3 bottom-2 h-10 rounded-2xl bg-[var(--ds-surface-card)]/35 blur-xl" />
              {/* Number badge */}
              <div className={`absolute top-2 left-2 w-6 h-6 ${activity.numBgGlass} ${activity.numTextGlass} rounded-full flex items-center justify-center font-black text-2xs shadow-[0_6px_16px_rgba(15,23,42,0.12)]`}>
                {activity.number}
              </div>

              {/* Stars pill */}
              <div className="absolute top-2 right-2 bg-[var(--ds-surface-card)]/80 rounded-full px-1.5 py-0.5 text-4xs font-black text-yellow-600 flex items-center gap-0.5 shadow-sm">
                ⭐ {stars}
              </div>
              ...
              <p className="relative font-black text-[var(--ds-text-primary)] text-2xs uppercase tracking-[0.14em] text-center leading-tight">
                {t(activity.titleKey)}
              </p>
              <p className="relative text-[var(--ds-text-secondary)] text-4xs text-center leading-snug mt-0.5 min-h-[24px]">
                {t(activity.subtitleKey)}
              </p>

              {/* Progress dot */}
              <div className={`relative mt-1 flex h-2.5 w-2.5 items-center justify-center rounded-full ${done ? "bg-[var(--ds-brand-primary)] shadow-[0_0_0_4px_rgba(22,163,74,0.16)]" : "bg-[var(--ds-surface-card-active)]"}`} />
            </motion.div>
          </Link>
        );
      })}
