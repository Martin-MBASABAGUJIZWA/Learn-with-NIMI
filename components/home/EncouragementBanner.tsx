  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex-1 text-center sm:text-left">
        ...
        <p className="text-[var(--ds-text-tertiary)] text-xs mt-0.5">
      ...
      <div className="flex gap-2.5 shrink-0">
        {ACTIVITY_KEYS.map((key, i) => (
          <motion.div key={key}
            ...
            <div className="w-10 h-10 bg-[var(--ds-surface-card)] border border-ds-border rounded-xl flex items-center justify-center text-lg">
              {ACTIVITY_EMOJIS[i]}
            </div>
            <span className="text-5xs font-bold text-[var(--ds-text-tertiary)]">{t(key)}</span>
          </motion.div>
        ))}
      </div>
    </motion.div>
