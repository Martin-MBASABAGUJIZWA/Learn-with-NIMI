    ...
    return (
      <div className="bg-[var(--ds-surface-card)] border border-ds-border p-5 shadow-ds-card" style={{ borderRadius: "var(--leaf-r-lg)" }}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-black text-ds-text text-lg">Controls</h2>
        </div>
        <div className="space-y-2.5">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-[58px] bg-[var(--ds-surface-card-active)] animate-pulse rounded-xl" />
          ))}
        </div>
      </div>
    );
        ...
        <AnimatePresence>
          ...
          {saving === "db" && !saved && (
            <motion.span key="saving" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="ml-auto flex items-center gap-1.5 text-2xs font-semibold text-[var(--ds-text-tertiary)]">
      ...
      <div className="space-y-2.5">
        ...
        <div className="flex items-center gap-3 p-3 bg-[var(--ds-surface-card-hover)] border border-ds-border" style={{ borderRadius: "var(--leaf-r)" }}>
          ...
          <div className="flex-1 min-w-0">
            <p className="text-ds-text text-sml font-black">Daily Goal</p>
            <p className="text-[var(--ds-text-secondary)] text-3xs">{dailyGoal} {dailyGoal === 1 ? "mission" : "missions"} per day</p>
          </div>
          <div className="flex items-center gap-1.5">
            <button onClick={() => setDailyGoalVal(Math.max(1, dailyGoal - 1))}
              className="w-8 h-8 rounded-lg bg-[var(--ds-border-primary)] text-[var(--ds-text-primary)] font-bold text-base flex items-center justify-center hover:bg-[var(--ds-surface-input)] active:scale-95 transition">−</button>
            <span className="text-ds-text font-black text-lg w-6 text-center tabular-nums">{dailyGoal}</span>
        ...
        <div className="flex items-center gap-3 p-3 bg-[var(--ds-surface-card-hover)] border border-ds-border" style={{ borderRadius: "var(--leaf-r)" }}>
          ...
          <div className="flex-1 min-w-0">
            <p className="text-ds-text text-sml font-black">Daily Time Limit</p>
            <p className="text-[var(--ds-text-secondary)] text-3xs">{screenTime} minutes of screen time</p>
          </div>
          <div className="flex items-center gap-1.5">
            <button onClick={() => setScreenTime(Math.max(10, screenTime - 10))}
              className="w-8 h-8 rounded-lg bg-[var(--ds-border-primary)] text-[var(--ds-text-primary)] font-bold text-sm flex items-center justify-center hover:bg-[var(--ds-surface-input)] active:scale-95 transition">−</button>
            <span className="text-ds-text font-black text-sm w-12 text-center tabular-nums">{screenTime}m</span>
        ...
        <div className="relative">
          <div className="flex items-center gap-3 p-3 bg-[var(--ds-surface-card-hover)] border border-ds-border" style={{ borderRadius: "var(--leaf-r)" }}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-ds-text text-sml font-black">Learning Language</p>
              <p className="text-[var(--ds-text-secondary)] text-3xs">{childName}&apos;s language</p>
            </div>
            <button
              onClick={() => setLangOpen(o => !o)}
              disabled={saving === "lang"}
              className="flex items-center gap-1.5 text-[var(--ds-text-primary)] font-black text-xs bg-[var(--ds-surface-card)] border border-ds-border px-3 py-1.5 rounded-full hover:bg-[var(--ds-surface-card-hover)] active:scale-95 transition disabled:opacity-50">
            ...
            {langOpen && (
              <>
                ...
                <motion.div
                  ...
                  className="absolute right-0 top-full mt-1 z-50 bg-[var(--ds-surface-card)] border border-ds-border shadow-lg rounded-2xl overflow-hidden min-w-[180px]">
                  {(["en", "fr", "rw"] as Language[]).map(lang => (
                    <button key={lang} onClick={() => handleLangChange(lang)}
                      className={`w-full text-left px-4 py-2.5 text-sml font-semibold transition hover:bg-[var(--ds-surface-card-hover)] flex items-center gap-2 ${language === lang ? "text-[var(--ds-brand-primary)] bg-[var(--ds-brand-subtle)]" : "text-ds-text"}`}>
                      {LANG_LABELS[lang]}
                      {language === lang && <span className="ml-auto text-3xs font-black text-[var(--ds-brand-primary)]">✓</span>}
                    </button>
                  ))}
                </motion.div>
              </>
