    ...
    <div className="bg-[var(--ds-surface-card)] border border-ds-border p-5 shadow-ds-card" style={{ borderRadius: "var(--leaf-r-lg)" }}>
      ...
      {!push.supported && (
        <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-2xl">
          ...
          <div>
            <p className="font-black text-amber-800 text-sml">Not supported in this browser</p>
            <p className="text-amber-700 text-2xs mt-0.5 font-nunito">
      ...
      {push.supported && push.permission === "denied" && (
        <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-2xl">
          ...
          <div>
            <p className="font-black text-red-700 text-sml">Notifications blocked</p>
            <p className="text-red-600 text-2xs mt-0.5 font-nunito">
        ...
        <>
          <div className="flex items-center gap-4 p-4 rounded-2xl border transition-colors"
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[var(--ds-text-secondary)] text-2xs mt-0.5 font-nunito">
                {push.isSubscribed
              ...
              {push.error && <p className="text-red-500 text-2xs mt-1 font-nunito">{push.error}</p>}
            </div>
            {push.loading ? (
              <Loader2 className="w-5 h-5 animate-spin text-[var(--ds-text-tertiary)] shrink-0" />
            ) : (
              <button
            ...
            {push.isSubscribed && (
              <motion.div
                ...
                <div className="mt-3 space-y-1.5">
                  <p className="text-3xs font-black text-[var(--ds-text-tertiary)] uppercase tracking-wide mb-2">What we notify you about</p>
                  {([
                    ...
                    <div key={item.key} className="flex items-center gap-3 px-3 py-2.5 bg-[var(--ds-surface-card-hover)] border border-ds-border rounded-xl">
                      <span className="text-base shrink-0">{item.emoji}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-ds-text">{item.label}</p>
                      ...
                      <button onClick={() => togglePref(item.key)}
                        ...
                        <motion.div
                          ...
                          transition={{ type: "spring", stiffness: 500, damping: 30 }}
                          className="w-5 h-5 bg-[var(--ds-surface-card)] rounded-full shadow" />
                      </button>
                    </div>
                  ))}
                </div>
                ...
                <button onClick={sendTest} disabled={testSent}
                  className="mt-3 w-full flex items-center justify-center gap-2 text-xs font-bold text-[var(--ds-text-secondary)] hover:text-[var(--ds-text-primary)] border border-ds-border hover:border-[var(--ds-border-strong)] rounded-xl py-2 transition disabled:opacity-50">
