  ...
  return (
    <div className="space-y-4 mt-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-baloo font-black text-ds-text text-lg">{t("activityProgressTitle")}</p>
          <p className="text-ds-muted text-sml mt-0.5">{t("activityProgressSubtitle")}</p>
        </div>
        <Link href="/user-profile/activity-details"
          className="text-xs font-bold text-[var(--ds-brand-primary)] hover:underline shrink-0 mt-1.5">
          {t("viewAll")} →
        </Link>
      </div>
      ...
      <div className="bg-ds-card border border-ds-border p-4 shadow-ds-card" style={{ borderRadius: 'var(--leaf-r)' }}>
        <div className="flex items-center justify-between mb-2">
          <span className="text-sml font-black text-ds-text">{t("profileOverallProgressLabel")}</span>
          <span className="text-sml font-black text-[var(--ds-brand-primary)]">{totalDone}/{totalActivities}</span>
        </div>
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-baloo font-black text-ds-text text-sm truncate">{t(activity.titleKey)}</p>
                ...
                <span className={`font-black text-sm shrink-0 tabular-nums ${isDone ? "text-amber-500" : "text-ds-text"}`}>
                  {isDone ? "🏅" : `${pct}%`}
                </span>
              </div>
