...
export default function MeetCharactersCard({ assets }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={SPRING.card}
      className="bg-[var(--ds-surface-card)] border border-ds-border leaf p-5 shadow-sm">
      <p className="font-baloo font-black text-[var(--ds-text-primary)] text-center text-mlg mb-4">Meet Your Story Friends! 👋</p>
      <div className="flex gap-3 justify-center">
        {chars.map((c, idx) => (
          <motion.div key={c.name}
            ...
            <p className="font-baloo font-black text-[var(--ds-text-primary)] text-sm">{c.name}</p>
            <p className="font-nunito text-[var(--ds-text-secondary)] text-3xs text-center leading-tight">{c.desc}</p>
          </motion.div>
        ))}
      </div>
